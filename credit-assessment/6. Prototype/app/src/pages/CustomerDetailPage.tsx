...
import { assessmentsForCustomerDivision, isReturnedForRevision } from "../store/selectors";
  ...
  return (
    <div>
      <div className="flex items-start justify-between mb-6">
        <SectionHeading eyebrow="FR8.5" title={customer.name} dek={`${customer.industry} · Relationship owner: ${customer.relationshipOwner}`} />
        <div className="flex gap-2">
          <Link to="/">
            <Button variant="secondary">← Directory</Button>
          </Link>
          <Link to={`/start?customerId=${customerId}&division=${encodeURIComponent(division)}`}>
            <Button title="FR8.5 → FR8.3: pre-selects this customer and division">New assessment for this customer</Button>
          </Link>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <Card>
            <SectionHeading eyebrow="FR8.6" title="Assessment history" dek={`Read-only drill-down into any prior assessment for ${division}.`} />
          ...
          <Card>
            <SectionHeading eyebrow="FR8.5" title="Assessments" dek="Read-only drill-down into any prior assessment's field, integrity-check, ratio, rating, and commentary state." />
        ...
        <Card>
          <SectionHeading eyebrow="FR1.9" title="Documents" dek="Uploaded per assessment — never shared across assessments (FR1.1)." />
