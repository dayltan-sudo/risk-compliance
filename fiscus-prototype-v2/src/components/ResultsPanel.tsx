...
import { Card, SectionHeading } from "./Card.js";
import { Badge } from "./Badge.js";
import { formatRatioValue, formatChangePct } from "../utils/format.js";
import type { Assessment, RatioKey } from "../types.js";
...
export function ResultsPanel({ assessment }: { assessment: Assessment }) {
  ...
  if (!rating) {
    return (
      <Card>
        <p className="text-[var(--muted)]">Not yet computed — every review item must be Confirmed or Amended first.</p>
      </Card>
    );
  }
  ...
  function ratioCell(key: RatioKey, period: string | null) {
    ...
    if (r.zeroDivisorTierApplied !== null) return <Badge tone="accent" title={`Zero divisor: ${r.zeroDivisorField} — tier ${r.zeroDivisorTierApplied} applied`}>— (zero divisor)</Badge>;
  ...
  return (
    <div className="space-y-6">
      <Card>
        <SectionHeading title="Ratio Calculation" dek="Closed formulas with full lineage to exact source field values." />
          ...
          <tbody>
            {RATIO_ORDER.map((def) => (
              <tr key={def.key} className="border-b border-[var(--line)] last:border-0 align-top">
                ...
                <td className="py-2">
                  <div className="text-[10px] text-[var(--muted)] font-mono mb-0.5">Change</div>
      ...
      <Card>
        <SectionHeading title="Derived scorecard inputs" dek="Computed alongside the ratios, consumed only by the scorecard." />
      ...
      <Card>
        <SectionHeading title="Scorecard & Rating" dek="Eleven criteria, tier × weight, composite 100–300. Quantitative and categorical — no qualitative override at MVP." />
