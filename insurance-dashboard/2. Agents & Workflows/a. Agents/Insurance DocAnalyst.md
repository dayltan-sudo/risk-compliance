...
| :--- | :--- | :--- | :--- |
| A — Intake & Classification | `salus_upload_document`, `salus_classify_document` | Document uploaded | Virus-scan, store immutably, assign `ingestion_id`, classify into one of ten `document_class` outputs |
| B — Extraction | `salus_extract_fields` | `document_class` resolved | OCR/NLP/IDP over the resolved field set; confidence + source pointer per field |
| C — Confidence Routing | `salus_queue_exception` | Confidence < threshold, or mandatory field missing | Pre-accept high-confidence fields; queue the rest with reason code + owner |
| D — Manual Questionnaire | `salus_submit_questionnaire` | No source document (FR3.9) | Preparer completes §14; checker review (different user) converges into Flow E |
| E — Human Validation | `salus_submit_validation` | Validator confirmed; Validation Convergence met | Confirms/corrects fields and optionally confidence; produces `validated_record` |
| F — Reprocessing | `salus_extract_fields` (re-run) | Re-run after model improvement (FR3.8, Could/V2) | New version; prior history retained |
| G — Contract-Requirement Direct Posting | `salus_post_requirement_input` | `record_type = CONTRACT_REQUIREMENT`; Validation Convergence met | Writes `app:contract_requirement_inputs` directly; triggers CoverageAnalyst's compliance recompute |
| H — Enrichment & Posting | `salus_post_record` | `record_type = POLICY_FIELD_SET`; Posting Convergence met | FX/geocode/risk enrichment; versioned write to `app:policy_registry`; triggers all three engine recomputes |
...
```
Trigger: `salus_upload_document` (entry point, no precondition). `salus_classify_document` (sole caller this agent; precondition: virus scan passed, `ingestion_id` assigned).
...
[Node 1: Preparer Completes Questionnaire] ──► Enters every mandatory
                 ...
                 │
                 ▼
[Node 2: salus_submit_questionnaire] ──► Precondition: all mandatory §14
...
[Entry: validated_record (record_type = CONTRACT_REQUIREMENT)]
                 │
                 ▼
[Node 1: salus_post_requirement_input] ──► Precondition: Validation
                                            Convergence met AND
...
```
Trigger: `salus_post_requirement_input`, sole caller this agent, precondition Validation Convergence met and `record_type = CONTRACT_REQUIREMENT`.
...
[Node 5: Enrichment Success Gate] ──► External calls retried/cached
    ...
    [Held; retry] [Node 6: Versioned Write] ──► Writes app:policy_registry
                  ...
                  [Node 7: Recalc Trigger] ──► Fires recalc_trigger_set to
                       ...
                       recompute (FR2.3)
```
Trigger: `salus_post_record`, sole caller this agent, precondition Posting Pre-Gate met (§7).
...
Preparer completes every mandatory §14 field in-system; `salus_submit_questionnaire` (sole caller this agent) writes `questionnaire_draft`, tagged 'manual entry — no source document', with no confidence score since a human typed the value. A different authorized user — never the preparer — then reviews each field against its supporting rationale and confirms or corrects it. Once every mandatory field is checker-confirmed, Validation Convergence (§6) is met and `salus_submit_validation` fires. Handoff to Flow H is identical to the extraction path.
...
**Pre-gate** — must hold before `salus_post_record` is called:
...
Both `posted_version_id` and the audit entry are produced by `salus_post_record` itself (Flow H Node 6) — the only path anywhere that writes `app:policy_registry`.
...
- **Parallel run:** each entity runs Salus alongside its existing local tracker for at least one renewal cycle after migration.
...
| `record_type` unresolved at Flow E Node 4 | `salus_submit_validation` blocked — handoff target must be known before validation completes |
...
Defines what `salus_extract_fields` must extract and score at Flow B, and what Flow H writes a field-level source-location pointer for (FR3.6). M = Mandatory, O = Optional. SOV = Statement of Values. Covers the six policy-side classes in §11.
