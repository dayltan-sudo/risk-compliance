...
[Output: signal_draft, PENDING] ──► held for confirm/dismiss gate,
                                     never auto-applied
```
Nodes 4–5 are Stretch/V2 tier (§6); their output still passes through the confirm/dismiss gate before touching anything else.
...
## 6. MVP Baseline vs. Stretch (PRD §17.3 Scope Risk)
...
| :--- | :--- | :--- |
| `atlas_ingest_news` | RiskScanner | MVP baseline (§6) — scheduled feed pull, no other precondition |
