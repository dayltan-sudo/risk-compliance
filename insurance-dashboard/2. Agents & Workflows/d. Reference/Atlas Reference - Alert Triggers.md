...
>
> **Companion docs:** routing logic — [`../a. Agents/Atlas Orchestrator.md`](../a.%20Agents/Atlas%20Orchestrator.md) §11 (Alerts & Notification, formerly a standalone workflow, now that agent's Flow F/G). State schema — [`../c. State/Atlas - Google ADK State Reference.md`](../c.%20State/Atlas%20-%20Google%20ADK%20State%20Reference.md).
...
| 7 | Emerging risk / appetite breach | News-driven signal implies exposure beyond risk appetite or coverage | R&C Manager, Group Insurance | **FR6.8**, gated by **FR6.5** | Must/**V2** |
...
Low-confidence extraction's human validation step also lets the reviewer confirm or adjust the extracted field's confidence level, not just its value — mechanism owned by [`../a. Agents/Insurance DocAnalyst.md`](../a.%20Agents/Insurance%20DocAnalyst.md) (Flow E, Node 1a).

**Reading this table:** rows 8 and 9 remain the only triggers directly traceable to a Must/MVP functional requirement. Row 7 is traceable but its content still depends on a V2 requirement — RiskScanner itself is MVP as of this rev (its baseline tier populates `app:news_signals` from go-live), but the trigger condition specifically names an *appetite breach*, which needs FR6.5 (appetite comparison), still Stretch/V2 on RiskScanner's own tiering (RiskScanner.md §6). A confirmed signal with no appetite-comparison data cannot satisfy this trigger's condition, so the row's Release stays Must/V2 even though the underlying agent no longer is. Rows 1–6 have a trigger condition and recipients specified in §12.1 but no corresponding FR row anywhere in §6 — no functional requirement was found for them, and none should be inferred. Their Release values are explicit **sponsor decisions dated 21 Jul 2026**, assigned directly against this table rather than traced to a PRD requirement. See [`../a. Agents/Atlas Orchestrator.md`](../a.%20Agents/Atlas%20Orchestrator.md) §11 for how the Alerts function itself is scoped in light of this.
...
Produced by the Reporting & Export function of [`../a. Agents/InsuranceCustodian.md`](../a.%20Agents/InsuranceCustodian.md) §3 — V2 (unlike that agent's other function, Audit & Access Log, which is MVP); none of these four rendered reports are available at MVP go-live, though the audit lineage report's underlying log is populated from MVP.
