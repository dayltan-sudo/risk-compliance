# Atlas — MVP Implementation Handoff

Everything needed to build the first cut of **Atlas** (Keppel Global Insurance Monitoring System). No prior context assumed — this folder is a complete, buildable subset of a larger design; nothing here references material you don't have.
...
**Read order:** this doc → `Atlas - Google ADK State Reference.md` → `Atlas - Data Lifecycle & Versioning Reference.md` → each component doc in build-sequence order (below).
...
| Atlas Assistant Orchestrator | Agent | Conversational Q&A over live data, with citations | [doc](a.%20Agents/Atlas%20Assistant%20Orchestrator.md) |
...
| 4 | Atlas Assistant Orchestrator — needs 1–3 built first; it has nothing to query otherwise |

## Decisions settled 21 Jul 2026 — build against these, not earlier drafts

| Area | Decision |
| :--- | :--- |
| Manual questionnaire (no source doc) | Maker-checker: a *different* user must confirm every field before it posts. New tool `atlas_submit_questionnaire`. |
