...
[Node 5: Enrichment Success Gate] ──► External calls retried/cached
    ...
    [Held; retry] [Node 6: Versioned Write] ──► Writes app:policy_registry;
                  ...
                  [Node 7: Recalc Trigger] ──► Fires recalc_trigger_set to
                       ...
                       engines (FR2.3)
```
Trigger: `salus_post_record`, sole caller Enrichment & Posting, precondition Validation Convergence met **and** enrichment succeeded.
...
All four conditions gate `salus_post_record`. A `validated_record` alone is not sufficient — enrichment must succeed and the version/audit write must complete before any downstream engine can treat the record as current.
...
- **Parallel run:** each entity runs Salus alongside its existing local tracker for at least one renewal cycle after migration before that tracker is retired, to catch migration gaps.
