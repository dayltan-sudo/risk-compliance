...
> Reference doc, Google ADK 2.0. Consolidated state schema for all twelve Atlas components — three in `a. Agents/` (two agents + one hybrid), nine deterministic workflows in `b. Workflows/`. Each component file links here instead of repeating its schema.
>
...
| :--- | :--- | :--- |
| Intake & Classification | Session: `ingestion_id`, `document_class`, `intake_status` | Sole writer of `app:document_store`. Assigns `ingestion_id` at `atlas_upload_document`. |
| Enrichment & Posting | Session: `validated_record`, `enrichment_result`, `posted_version_id`, `recalc_trigger_set` · `temp:fx_lookup` | Sole writer of `app:policy_registry` (rule 1 above). Calls `atlas_post_record` only on Posting Convergence; triggers all three Engines. |
