# Salus — Data Lifecycle & Versioning Reference
...
>
> **Companion docs:** [`Salus - Google ADK State Reference.md`](Salus%20-%20Google%20ADK%20State%20Reference.md) · [`Salus - Output Templates.md`](Salus%20-%20Output%20Templates.md)

## The Core Pattern: Valid-Time vs. Transaction-Time

Salus needs two time axes per non-reference record — bitemporal, not the full academic version:
...
These diverge: a broker's July correction can restate what was true in March. Salus must answer both "what was true in March" and "what did we believe then, versus now" — the second is what an auditor traces.
...
> **Resolved (sponsor, 21 Jul 2026):** Salus continues tracking an open claim after the policy it was filed against has lapsed and been superseded by a new placement. Tracking ends at claim settlement, not at policy expiry — consistent with the claim's own independent lifecycle clock (§7.4).
...
| Migration | 1 renewal cycle, one-time, per entity | §9.2: Salus and legacy tracker run concurrently before cutover. |
...
**Versioned entities** (Asset, Policy, Coverage, Premium, ExtractionField, Third-Party Requirement, Policy Exclusion): type-2 SCD — `valid_from`/`valid_to` (real-world period) plus `recorded_at` (when Salus learned it); current row has open-ended `valid_to`. A correction closes the old row, inserts a new one.
...
- Dismissed news-signal retention → retained indefinitely, archived from the active watchlist *(News agent detail — V2, not in this handoff)*.
