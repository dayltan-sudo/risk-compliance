# Salus Reference: Alert Triggers
...
>
> **Companion docs:** routing logic — [`../b. Workflows/Alerts & Notification.md`](../b.%20Workflows/Alerts%20%26%20Notification.md). State schema — [`../c. State/Salus - Google ADK State Reference.md`](../c.%20State/Salus%20-%20Google%20ADK%20State%20Reference.md).
...
Low-confidence extraction's human validation step also lets the reviewer confirm or adjust the extracted field's confidence level, not just its value — mechanism owned by [`../a. Agents/Field Extraction & Validation Routing.md`](../a.%20Agents/Field%20Extraction%20%26%20Validation%20Routing.md) (Flow D).

**Reading this table:** rows 8 and 9 remain the only triggers directly traceable to a Must/MVP functional requirement. Row 7 is traceable but to a V2 requirement — it cannot fire before `app:news_signals` exists, which depends on the News & Sector Intelligence Agent. Rows 1–6 have a trigger condition and recipients specified in §12.1 but no corresponding FR row anywhere in §6 — no functional requirement was found for them, and none should be inferred. Their Release values are explicit **sponsor decisions dated 21 Jul 2026**, assigned directly against this table rather than traced to a PRD requirement. See [`../b. Workflows/Alerts & Notification.md`](../b.%20Workflows/Alerts%20%26%20Notification.md) §3 for how the Alerts & Notification workflow itself is scoped in light of this.
...
Produced by [`../b. Workflows/Reporting & Export.md`](../b.%20Workflows/Reporting%20%26%20Export.md); this component's own MVP placement is likewise inferred, not PRD-tagged — see that doc's §3.

### 2.3 Export formats
PDF and Excel; distributed on demand or on schedule. Role-based PII redaction and source-document export restriction apply to every export regardless of format (§13) — see [`../d. Reference/Salus Reference - RBAC & Access Scoping.md`](Salus%20Reference%20-%20RBAC%20%26%20Access%20Scoping.md).
