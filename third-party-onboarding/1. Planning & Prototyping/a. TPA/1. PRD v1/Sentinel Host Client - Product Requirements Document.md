...
**Working name for the combined application: KAI Sentinel.** The end-state is **one host client, several processes, one switcher** — not a separate app per process. TPA (this PRD) is the first process built on the shell; **FM&I KYC** is the second, specified in its own sibling PRD (`Onboarding Host Client - FM&I KYC - Product Requirements Document.md`) that plugs into the shell defined here rather than redefining it. Further processes (e.g. the non-MAS/Non-FM&I KYC process, KYC plan §6 item 4) are expected to follow the same pattern once scoped. Final ownership of the "KAI Sentinel" name/branding is open — see §10.
...
- **Branding/visual design ownership** — not specified here; this PRD covers structure and behaviour, not visual design. Includes confirming **"KAI Sentinel"** (§8) as the standing name for the combined multi-process application, and what (if anything) "KAI" stands for.
