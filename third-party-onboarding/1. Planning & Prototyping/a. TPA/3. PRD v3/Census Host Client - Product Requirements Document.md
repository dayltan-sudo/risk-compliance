# KAI Census Host Client — TPA — Product Requirements Document
...
6. Record commits. The client states plainly that the record is committed in Census. **No export is produced** — the record is viewable and reviewable in the client (§9.4, §9.5, §9.7).
...
5. **UI prototype** (`census-host-client-uiux-prototype.html`) is built against v9 and is now substantially wrong — R&C sign-off, RCTP export, role-gated views. Rebuild or retire.
