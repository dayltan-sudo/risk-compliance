...
| **Date** | 17 July 2026 (v1 · v1.1 · v1.2 · v1.3 · v1.4 · v1.5 · v1.6 · v1.7) · 22 July 2026 (v1.8) · 3 September 2026 (v2) |
| **Status** | Draft v2 — **reconciled against TPA PRD v10**, which simplified the shell this PRD plugs into: one persona (no R&C reviewer role), no Business Unit access control, no Dow Jones RCTP integration of any kind. Every place this PRD said "mirrors TPA" or "reuses TPA PRD §X unchanged" inherited those shell facts implicitly and is now wrong until corrected — see the v2 changelog entry for the itemised list. This pass changes **shell-inherited facts only**; it makes no new judgment calls on FM&I KYC's own content (the checklist, CTC detection, CDD typing) beyond what naming a single persona and removing RCTP requires. |
| **Audience** | Product / front-end team building the host client (chatbot) |
| **Sibling document** | [`Census Host Client - Product Requirements Document.md`](../a.%20TPA/3.%20PRD%20v3/Census%20Host%20Client%20-%20Product%20Requirements%20Document.md) — the **TPA PRD, v10**, in the sibling `a. TPA/3. PRD v3` folder. Read that one first. It defines the host client shell (three-pane layout, identity model, confirmation-gate pattern, product principles, confidence scale) that this document reuses rather than re-specifies. This PRD only defines what's **different** for the FM&I KYC process. |
...
|---|---|---|
| **v2** | 3 Sep 2026 | **Reconciled against TPA PRD v10**, which removed the R&C reviewer role, BU access control, and the Dow Jones RCTP integration from the shell. Changes, all shell-inherited rather than independently decided: (1) **§3 Personas** — collapsed to one Requester persona; the R&C reviewer row is gone, and everything it owned (CTC eligibility review when v2/KIV work resumes, review pack, exception report) now falls to whichever Requester is handling the case, same as TPA. **Flagged, not silently absorbed:** certifier-eligibility is a MAS-regulated judgment (§2.1) that used to sit with a second-line reviewer; collapsing it onto the same person who ran the case is a real change in who checks it, worth R&C's explicit sign-off before build, not just a mechanical persona merge. (2) **§6 Identity & role** — no role/BU confirmation prompt, no BU-scoped visibility; a case's BU is a displayed field, not an access boundary, mirroring TPA PRD §3. (3) **§7.2/§7.3/§7.4 handoff messages** — rewritten. RCTP is gone from the platform entirely (TPA PRD v10 §1); "staged in RCTP as [case reference], not yet submitted" is replaced with "committed in Census," the same honest-handoff posture TPA now uses. (4) **§7.9** — dropped the *(R&C)* tag; review pack gains the Source-match / Edited? columns TPA PRD §9.5 now specifies; the exception report is noted as largely redundant with §7.5's own checklist status view and, consistent with TPA cutting its own exception report from MVP, is not treated as a firm MVP commitment here either. (5) **§7.11** — failure-state list corrected to what TPA PRD v10 §10 actually still has (screening-unavailable park-and-retry, upload rejected, extraction weak/empty, needs-checking judgment field); "write denied — BU/role," "entitlements stale," and "RCTP unavailable" are gone because the things that produced them are gone. (6) **§12** — dropped the DJ RCTP entitlements-export dependency. No change to this PRD's own content — the checklist, CDD typing, CTC detection, and case lifecycle are unaffected; KYC cases have no TPA-style expiry/renewal/amend concept and none is invented here. |
| **v1.8** | 22 Jul 2026 | **Added explicit honest-handoff message copy** for all three write-backs — §7.2 step 6 (`kyc_open_case`), §7.3 (`kyc_submit_cdd_typing`), §7.4 (`kyc_submit_wave2_documents`) — mirroring TPA PRD §7.2 step 6's pattern, which none of this PRD's three write-backs had until now (they only inherited the §2.1 *principle*, never got the actual scripted line). **Made §7.9's field-volume treatment explicit**: a case reaching review carries up to ~39 items across all three stages combined, inside TPA PRD §5's "must not be a flat list" range — grouping + mandatory-only filter now stated directly rather than left to be inferred from "mirrors TPA." Both gaps surfaced by a Census test walkthrough that, for the first time, wrote out actual chat/canvas copy against these files instead of treating every human-in-the-loop step as an abstract confirmation. No behavioural or data change — same posture as v1.7. |
...
This PRD **inherits the TPA PRD §2 principles**, as v10 restated them: review-not-create, no-fabrication on judgment fields, and never claiming an unobserved outcome (v10's reworded honest-handoff — "nothing reads as screened, cleared, or committed until it is"). **Note the one TPA principle that changed underneath this PRD's own §2.1 stance, below:** TPA v10 dropped "evidence, not verdicts" as a named principle and now allows a source-match indicator (`agrees`/`differs`/`no source`) in its review pack. This PRD's own no-fabrication-on-document-status stance (last bullet below) is stricter than that by design — a document-checklist verdict is closer to TPA's judgment fields than its factual ones, so it keeps the blank-until-verified posture regardless of what TPA now allows elsewhere. It adds the following, specific to FM&I KYC:
...
**One user role, mirroring TPA PRD §3.** Every signed-in user is a Requester with identical capabilities: start/find a case, upload or forward customer documents, monitor chase progress, check status, complete the CDD-typing questionnaire, review the checklist and review pack, resolve discrepancies, and — once that workflow is prioritized (§7.7) — send customer rectification emails. There is no second-line reviewer role and no role-gated menu.

| Persona | Who | What they do in this client |
|---|---|---|
| **Requester** | Everyone who signs in | Everything above |
...
**Consequence worth R&C's explicit attention, not just a mechanical merge (v2, 3 Sep 2026):** §7.6 names certifier-eligibility as a MAS-regulated judgment — deliberately never auto-decided by the agent, always a human call. Before this PRD's single-role collapse, that human call sat with a distinct second-line reviewer; now it falls to whichever Requester happens to be running the case, the same person whose own case is being checked. TPA's collapse to one role was a defensible simplification for its own risk-scoring gate; whether the same collapse is acceptable for a **regulated certifier-eligibility judgment** is a compliance question this document cannot answer on TPA's authority alone. Flagged in §10 as an item needing R&C's own sign-off, not carried over silently.

Role resolution (AD/Entra SSO, server-side) mirrors TPA PRD §3/§6 — no role or BU confirmation prompt (§6 below).
...
**Reuses TPA PRD §3's simplified identity model.** Sign-in is native AD/Entra SSO — no separate login, no role or Business Unit confirmation prompt, because there is no role to confirm and no BU boundary to enforce. A case may still carry an owning Business Unit as a **displayed field**, exactly as TPA now treats it (data, not access control) — every user sees every case.
...
4. Items requiring certification run through the factual completeness check automatically (§7.6), resolving directly to Present/Non-CTC — certifier-eligibility characterization is deferred to v2 (KIV); a Non-CTC item today is left for the case's own Requester to assess manually (§3).
5. Any Missing or Non-CTC item feeds a draft rectification email (§7.7) — never auto-sent.
6. **Explicit handoff message (updated 3 Sep 2026 — RCTP no longer exists as a destination, TPA PRD v10 §1):** *"Wave 1 is committed in Census as [case reference]. Any open mandatory items carry forward as outstanding — confirming this draft doesn't require them to be resolved first. CDD Typing is next."* There is no external system this commits to and no separate submission step; Census is the case's system of record, same posture as TPA's own onboarding commit (TPA PRD §9.2 step 6). The client never implies Wave 1 is complete just because it's confirmed and committed.
...
**Explicit handoff message (updated 3 Sep 2026):** *"CDD Typing is committed in Census as [case reference]. Resolved tier: [Simplified/Standard/Enhanced]."* For Simplified, add: *"No Wave 2 applies — this case moves straight to review."* For Standard/Enhanced, add: *"Wave 2 is next."*
...
**Explicit handoff message (updated 3 Sep 2026):** *"Wave 2 is committed in Census as [case reference]. Any open items remain outstanding — everything that will be requested for this case has now been asked for; what's left needs the customer, not another wave."*
...
**⚠️ Certifier-eligibility characterization and translation-adequacy flagging (the two judgment bullets below) are deferred to v2, KIV (confirmed 17 Jul 2026).** The bullets below describe the full intended design; only the first three (factual, agent-executed today by `KYC DocReviewer`) are in the current build. A `Non-CTC` item today is simply left for the case's own Requester to assess manually, end to end (§3) — there is no agent-surfaced "needs eligibility review" flag or dedicated triage view yet. The `CTC Reviewer` agent that would perform the deferred bullets is kept as a preserved design (`3. Agentic Workflows/b. FM&I KYC/CTC Reviewer.md`) for when this is reprioritized.
...
- **Certifier eligibility (judgment, never auto-decided — deferred to v2, KIV):** even with a complete, dated, non-self certification, whether the certifier's stated professional capacity actually qualifies under the eligible-certifier list (embassy/consulate/high-commission officer, Commissioner of Oaths, lawyer/notary of a recognised body, judiciary member, accountant or company secretary of a recognised body, or a licensed FI's compliance officer) is a **regulated judgment under MAS rules**, never auto-decided (§2.1). When this is built, the item would be flagged "needs eligibility review" for review by someone other than the case's own Requester — the exact reason §3 flags this collapse for R&C's separate confirmation; today, the case's Requester reaches this same judgment manually off the plain `Non-CTC` status, with no agent-extracted certifier detail to assist.
...
### 7.9 Review pack — `kyc_review_pack` *(proposed)*
**No longer R&C-only (v2, 3 Sep 2026)** — available to any Requester, mirroring TPA PRD §9.5. Mirrors that section's table exactly: **Item | Value | Source reference | Confidence | Source match | Edited?** — `Source match` (`agrees`/`differs`/`no source`) set at extraction and re-evaluated on amendment; `Edited?` drawn from the case's confirmation log, same mechanics as TPA. Applied to the FM&I KYC property set (`4. Properties/Combined Consolidated Questionnaire Questions.xlsx`, `FM&I` tab; see §12) instead of TPA's.

**Field-volume treatment inherits from TPA PRD §5:** a case reaching review carries Wave 1 (9 items) + CDD Typing (19 questions) + Wave 2 (up to 11 items) — up to ~39 items combined. Grouped by wave/section, with a mandatory-review-only filter, not one continuous table.

**The standalone exception report is dropped, not carried forward (v2).** TPA PRD v10 cut its own exception report from MVP as largely redundant with the confirmation gate's own gap-flagging; the same redundancy is sharper here, since §7.5's checklist view already **is** an exception-style artifact — every item already carries a live Present/Missing/Non-CTC status, which is what an exception report would otherwise exist to summarise. `kyc_exception_report` is removed from Appendix A. If a genuine gap between the checklist view and a dedicated pre-review summary shows up in use, revisit — but it is not assumed here.
...
**Reuses TPA PRD v10 §10, corrected to what's actually still there (v2, 3 Sep 2026).** "Write denied — BU/role," "identity unverifiable," and "entitlements stale" are gone with the role/BU model that produced them; "RCTP unavailable" is gone with RCTP. What remains, shared with TPA:

| State | What the user sees |
|---|---|
| **Screening unavailable** | The case's screening step is parked, not lost — confirmed items are kept, screening retries in the background, and the user is told they'll be notified. Nothing commits until it completes. Mirrors TPA PRD §9's park-and-retry behaviour exactly. |
| **Upload rejected** | Per-file error; other files proceed. |
| **Extraction weak or empty** | Shown blank, "couldn't extract — enter manually." Never a fabricated fill. |
| **Needs-checking judgment field** | Blank, flagged "needs confirmation," citation still linked. |

Adds, specific to this process:
...
The combined application's working name, **KAI Census**, and the process-switcher requirement live in the TPA PRD as the shell-owning document; this PRD assumes both and does not restate them.
...
- **"KAI Census" as the standing name for the combined application** — confirmed as the working name to carry forward (see TPA PRD §8 edit accompanying this PRD); final ownership of branding is still open (TPA PRD §10).
- **Certifier-eligibility judgment now sits with the same Requester who ran the case, not a second-line reviewer (v2, 3 Sep 2026 — see §3).** This followed automatically from TPA's single-role collapse; it was never independently evaluated against MAS's regulated-judgment expectation for who checks a CTC certifier. Needs R&C's explicit confirmation before build, not just this document's say-so — a KYC-specific case for keeping a second-line check here, even without a persisted "role," is stronger than TPA's own case for dropping it.
...
- **AD/Entra SSO** — reused from TPA PRD §14, no KYC-specific change. **The DJ RCTP entitlements export is no longer a dependency of anything** — TPA PRD v10 removed RCTP from the platform entirely, and this document never had an independent reason to keep it.
...
- **Deferred (confirmed 17 Jul 2026 — not prioritized, revisit later):** the rectification-email draft/send workflow (§7.7), and **certifier-eligibility characterization + translation-adequacy flagging (§7.6, KIV for v2)** — the `CTC Reviewer` agent design is preserved but not built. The checklist and CTC factual-detection that inform both stay in MVP; only the client-native drafting/send tooling and the eligibility-characterization step are deferred, and the case's Requester handles both manually until they're built (§3).
