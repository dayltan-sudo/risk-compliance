# Sentinel Test Run 7 — Renewal Case (Kestrel Bay Industrial Services Pte. Ltd.)

First test of the renewal path (Prompt 9) — every prior test (1–6) exercised fresh onboarding (Prompt 1) only. Scenario: Kestrel Bay was onboarded ~18 months ago (`Sentinel Output 5`, Scenario A — assume the Host Client Confirmation Gate resolved, `TPA Interaction with Third Parties` was completed by the Requester at that time, and the record was staged as `TPA-KESTREL-001`). New documents have now been submitted for its renewal cycle. Simulated end-to-end by Claude; `[SIMULATED]` marks the Host Client Confirmation Gate and RCTP write-back.
