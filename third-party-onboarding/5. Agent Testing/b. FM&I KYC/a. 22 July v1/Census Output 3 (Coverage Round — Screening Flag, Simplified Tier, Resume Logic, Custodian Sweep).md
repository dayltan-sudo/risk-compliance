# Census Test Run 3 — Coverage Round (Findings 3 & 4 from `Census Output 2`)

Run against the current files, **after** fixing `Census Output 2`'s Findings 1 & 2 (`Agents/KYC DocReviewer.md`, `Workflows/KYC DocReviewer - Flows.md` — both edited immediately before this run). Covers the four paths `Census Output 2` flagged as never exercised:
...
**`kyc_submit_cdd_typing`:** Yes to Q5 → **`resolved_cdd_tier = Enhanced`** (second distinct Enhanced-trigger path exercised across the test suite — `Census Output 2`'s Silvercrest triggered via Q12; this one via Q5).
...
Continuing **Silvercrest Asset Management** (`Census Output 2`): its Wave 1 *was* confirmed and staged (`case_id = KYC-SILVERCREST-001`, in the registry), then the session ended before Flow C started — case sits at "Waiting on screening resolution."
...
**Result: confirmed working as specified.** This is the mechanism `Census Output 1`'s Flow A already exercised in the abstract ("no candidate match" case); this is its first test of the *match-found* branch, and it correctly resumes at the gate rather than restarting the case.
...
Portfolio at this point in the test suite: **Silvercrest** (Enhanced, staged, sitting at Wave 2 with 3 open gaps per `Census Output 2`), **Raffles Harbour Pension Trust** (Simplified, `Case Complete`), **Meridian Trust Services** (never staged — per Test 3a's finding, invisible to this sweep).
...
2. Chase items 3.3 (AML/CFT undertaking + UBO declaration), 3.5 (UBO proof of address), 5.3 (UBO SOF/SOW documentary proof) — per `Census Output 2`'s own Conditions C3/C4.
...
| Finding (from `Census Output 2`) | Result |
| :--- | :--- |
